...
def insert_data(table_name: str, file_path: str, postgres_conn_id: str):
    ...
    for index, row in df.iterrows():
        insert_sql = f"INSERT INTO {schema}.{table_name} (brand, model, year, mileage) VALUES (%s, %s, %s, %s);"
        pg_hook.run(insert_sql, parameters=[row['brand'], row['model'], row['year'], row['mileage']])
        count += 1
